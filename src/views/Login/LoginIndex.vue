  ...
  agree: [
    {
      validator: (rule, val, callback) => {
        // 这个 validator 确实只在 form 调用 validate 方法时才会执行
        // 当 agree 值为 true 时验证通过，否则返回错误信息
...
const handleLogin = () => {
  ...
  formRef.value.validate(async (valid) => {
    if (valid) {
      const code = await useUserInfoStore().getUserInfo(username, password)
      if (code == '1') {
        ...
        <div class="account-box">
          <div class="form">
            ...
            >
              ...
              <el-button size="large" class="subBtn" @click="onClick">点击登录</el-button>
            </el-form>
          </div>
        </div>
      </div>
